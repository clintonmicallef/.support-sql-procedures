              ...
              avgsettlementtime AS(
                ...
                FAIL as (
                  SELECT (to_char(unsettled.datestamp, 'YYYY-MM-DD HH24:MI'::text))::timestamp(0) AS datestamp,
                         ...
                         (SELECT bankaccounts.externalaccountholder FROM bankaccounts WHERE bankaccounts.transferbankaccountid = unsettled.totransferbankaccountid ORDER BY (bankaccounts.currency = unsettled.currency) DESC LIMIT 1) AS externalaccountholder,
          ...
          WITH unsettled AS(
           ...
           /*NEW*/ bankaccounts.externalaccountholder
             ...
             /*NEW*/ AND unsettled.bankaccountid in (
                ...
                SELECT fail.*, fail_deposit_transfer(fail.transferid)
                  FROM fail;
      ELSIF _tofail = 'no'
          THEN
          RAISE NOTICE 'No action. exiting!';
